#!/usr/bin/env python3
"""
Visualize detailed memory breakdown for HuggingFace DiT DP-SGD experiments
"""
...
def plot_memory_breakdown_comparison(results, output_dir):
    ...
    ax1.set_title('HuggingFace DiT: Detailed Memory Breakdown by Component', fontsize=16, fontweight='bold')
    ax1.set_xticks(x)
    ...
    if "vanilla" in results:
        ...
        if dp_methods:
            ...
            ax2.set_title('HuggingFace DiT: DP-SGD Memory Overhead Breakdown', fontsize=16, fontweight='bold')
            ax2.set_xticks(x2)
...
def plot_memory_timeline(results, output_dir):
    ...
    for idx, (exp_key, ax) in enumerate(zip(EXPERIMENT_ORDER, axes)):
        ...
        ax.set_title(f"HuggingFace DiT: {NAME_MAPPING[exp_key]}", fontsize=14, fontweight='bold')
...
def plot_performance_comparison(results, output_dir):
    ...
    ax.set_title('HuggingFace DiT: Memory vs Training Time', fontsize=18, fontweight='bold')
...
def generate_summary_table(results, output_dir):
    ...
    summary_lines.append("HuggingFace DiT: DETAILED MEMORY PROFILING SUMMARY")
    summary_lines.append("="*100)
    summary_lines.append("")
    summary_lines.append("Model Configuration:")
    summary_lines.append("  - Model: facebook/dit-large (via HuggingFace transformers)")
    summary_lines.append("  - Image Size: 1024x1024")
    summary_lines.append("  - Patch Size: 8x8")
    summary_lines.append("  - Number of Tokens: 16,384")
    summary_lines.append("  - Batch Size: 1")
    summary_lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Visualize HuggingFace DiT memory breakdown")
    ...
    print(f"\n{'='*80}")
    print("Loading HuggingFace DiT experiment results...")
    print(f"{'='*80}\n")
