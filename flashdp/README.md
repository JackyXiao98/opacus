...
You can also easily use [Opacus](https://github.com/pytorch/opacus/) to compute hyperparameters, or compute hyperparameters by yourself.
```python
...
<details>
<summary>We welcome contributions from the community! Click to see the steps on how to contribute.</summary>

1. **Fork the Repository:** Click on the 'Fork' button at the top of this page.
2. **Clone the Fork:** `git clone https://github.com/<your_username>/flash_dp.git`
...
6. **Push to the Branch:** `git push origin your_new_branch_name`
...
1. **Enhanced AMP Support** We are committed to accelerating the integration of Automatic Mixed Precision (AMP) within FlashDP.
2. **Support for Additional Layer Types** Extending FlashDP’s applicability by adding support for more layer types such as Conv2D and Conv3D.
