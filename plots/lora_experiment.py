#!/usr/bin/env python3
"""
Run a single profiling experiment with a custom Transformer model with LoRA.
...
class TransformerLayerWithFlashAttention(nn.Module):
    ...
    def __init__(self, hidden_dim: int, num_heads: int, dropout: float = 0.0):
        ...
        # Use explicit modules instead of Sequential for LoRA targeting
        self.ffn_up = nn.Linear(hidden_dim, hidden_dim * 4)
        self.ffn_act = nn.ReLU()
        self.ffn_down = nn.Linear(hidden_dim * 4, hidden_dim)
    ...
    def forward(self, x):
        ...
        x_ = self.ln2(x)
        # FFN forward pass
        x_ffn = self.ffn_up(x_)
        x_ffn = self.ffn_act(x_ffn)
...
class TransformerModelWithFlashAttention(nn.Module):
    ...
    def __init__(
        ...
        # Add a simple config attribute for PEFT compatibility
        # PEFT may access this to understand model structure
        class SimpleConfig:
            def __init__(self, vocab_size, hidden_dim, num_layers, num_heads):
                ...
                self.model_type = "custom"  # PEFT checks this
                self.tie_word_embeddings = False  # PEFT checks this
            ...
            def __getattr__(self, name):
                """Allow attribute access, fallback to dict"""
                if name in self._dict:
                    return self._dict[name]
                raise AttributeError(f"'{self.__class__.__name__}' object has no attribute '{name}'")
        
        self.config = SimpleConfig(vocab_size, hidden_dim, num_layers, num_heads)

    def forward(self, input_ids=None, labels=None, attention_mask=None, *args, **kwargs):
        """
        Forward pass of the transformer model.
        Accepts attention_mask and other args/kwargs for PEFT compatibility,
        even though we don't use them in this simple implementation.
        """
        # Handle case where input_ids might be passed as first positional arg
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, attention_mask=None, use_cache=True, **kwargs):
        """
        Stub method required by PEFT for TaskType.CAUSAL_LM.
        We don't actually use generation, so this is just for compatibility.
        """
...
def setup(rank, world_size):
    ...
    os.environ["MASTER_PORT"] = "12357"  # Different port from other experiments
...
def create_model(config, master_process=True):
    """Create custom transformer model with LoRA"""
    if master_process:
    ...
    # For our custom transformer: attention projections, FFN linear layers, and output head
    # PEFT will match these module names in the model hierarchy
    target_modules = [
        "q_proj", "k_proj", "v_proj", "out_proj",  # Attention projections
        "ffn_up", "ffn_down",  # FFN layers (up and down projections)
        "lm_head",  # Output head
    ]
...
def run_experiment_worker(
    ...
    seq_length = config["seq_length"]
    batch_size = config["batch_size"]
    use_autocast = config.get("bf16", False)
    autocast_kwargs = {"dtype": torch.bfloat16, "enabled": use_autocast}
    ...
    if master_process:
        ...
        print(f"{'='*80}\n")
        print(f"Mixed precision (bf16 autocast): {use_autocast}")
    ...
    # Note: This should be done after LoRA is applied, but PEFT models may need special handling
    if is_fuse_mode:
        if master_process:
            print("Warning: Fuse mode with LoRA may have compatibility issues")
    ...
    for i in range(warmup_iter):
        ...
        with torch_amp.autocast("cuda", **autocast_kwargs):
...
def run_experiment(config):
    ...
    print(f"  bf16 Autocast: {config['bf16']}")
    print(f"{'#'*80}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Run single transformer with LoRA profiling experiment")
    ...
    parser.add_argument("--bf16", dest="bf16", action="store_true", default=True,
                       help="Enable bf16 autocast (default: enabled)")
