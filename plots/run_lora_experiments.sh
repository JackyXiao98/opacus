#!/bin/bash
# Run all Transformer with LoRA memory profiling experiments in isolated Python processes
# This prevents memory pool contamination between experiments

echo "========================================================================"
echo "Transformer with LoRA Memory Profiling Experiment Suite"
...
HIDDEN_DIM=768         # GPT-2 Large 的 hidden size
NUM_LAYERS=12           # 36 层 transformer block
NUM_HEADS=12            # 20 个 attention heads

# Configuration - LoRA
LORA_RANK=8             # Fixed LoRA rank (can be changed to array if needed)
...
# Sequence lengths to test (PRIMARY VARIABLE)
...
MODES=("ghost" "ghost_bk" "flash_fuse" "flash_fuse_bk" "no_dp_single")
...
run_experiment() {
    local mode=$1
    local seq_len=$2
    local output_file="$RUN_DIR/${mode}_seq${seq_len}_lora${LORA_RANK}_result.json"
    
    echo "========================================================================"
    echo "Running: mode=$mode, seq_length=$seq_len, lora_rank=$LORA_RANK"
    ...
    if [ $exit_code -ne 0 ]; then
        ...
        echo "✓ Completed: mode=$mode, seq_length=$seq_len, lora_rank=$LORA_RANK"
    fi
...
for seq_len in "${SEQ_LENGTHS[@]}"; do
    for mode in "${MODES[@]}"; do
        result_file="$RUN_DIR/${mode}_seq${seq_len}_lora${LORA_RANK}_result.json"
